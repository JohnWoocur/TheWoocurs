...
if(!empty($_GET['pid'])) {
    $pid = $_GET['pid'];
    $result = $mysqli->query("SELECT * FROM `job` WHERE `id` = $pid");
    if($result && $result->num_rows) {
       $row = $result->fetch_assoc();
	   if($row['e_user'] != $_SESSION['USER_ID']) {
		session_destroy();
		header("Location: login.php");
		exit;
		}
...
$user = $mysqli->query("SELECT * FROM user WHERE `id` = $cid");
...
<html lang="en-us">
	...
	<body id="body" class="body">
			...
			<ul class="menu" id="menu">		
				...
				<li class="navspace"><a href="ceditprofile.php?cid=<?=$cid?>"><?=$userrow['name']?><img src="img/user-2.png" style="width:15px; height:15px;"></a></li>
		...
		<div id="index" class="index"><br>
			<div class="box">
				<form id="postForm" action="include/<?=($pid?'update':'add')?>-post.php" method="post" enctype="multipart/form-data">
					<h3><?=($pid?'Edit':'Post New')?> Project</h3>
					...
					<div>
						<div>
							<label for="prjplace">Choose Place of your project</label>
					...
					<div>
						<div>
							...
							<input type="text" id="cost" name="cost" placeholder="Project Cost in LKR" required value="<?=@$row['cost']?>" min="100" max="100000">
						</div>
					</div>
					<div>
						<div>
