...
$pid = (int) @$_GET['pid'];
$result = $mysqli->query("SELECT `id`, `title`, `description`, `cost`, `project_leader`, `f_user`, `date`, `place` FROM project WHERE `id` = $pid");
if(!$result->num_rows) {
    exit("No Deals found.");
}
...
if(isset($_POST["ez"])){
	$id = $_POST['id'];
	$refno = $_POST['refno'];
	$resultpay = $mysqli->query("UPDATE `project` SET `payment_file` = '$refno', `payment` = '1', `method` = 'ezcash' WHERE `id` = '$id'");
    if($resultpay==true){
...
if(! (file_exists('user_data') && is_dir('user_data')) ) {
	...
	$result = $mysqli->query("UPDATE `project` SET `payment_file` = '$file_path', `payment` = '1', `method` = 'bank' WHERE `id` = '$id'");
    if($result==true){
...
<html lang="en-us">
	<head>
		...
		<!--Style sheets css -->
		<link rel="stylesheet" href="css/dealsview.css">
		<!--End of css -->
		...
		<br>
			<div class="box">
                ...
                <div>
                    ...
                    <p><?=$row['place']?></p>
					<h4>Deals Deadline</h4>
                    <p><?=$row['date']?></p>
					<p><?=($project?'Deal Closed':'')?></p>
                    <h4>Bid Proposal</h4>
                    <?php
                    if($_SESSION['USER_TYPE'] == 'freelancer' AND !$confirm) {
                        $query = "SELECT * FROM `members_request` WHERE `project_id` = $pid AND (`member_id` = $uid OR `status` = 'accepted')";
                    ...
                    <form id="bidForm" action="include/uploaddeals.php" method="post" novalidate enctype="multipart/form-data">
                        ...
                        <div>
                            ...
                            <?php
							$transaction = $mysqli->query("SELECT * FROM `project` WHERE `id` = '$pid'");
                    ...
                    ?>
                    <form id="bidForm" action="include/placebiddeals.php" method="post" novalidate>
                        <input type="hidden" name="pid" value="<?=$row['id']?>">
                        <div>
                            <div>
                    ...
                    <?php
                        ...
                        $res = $mysqli->query("SELECT * FROM `members_request` WHERE `project_id` = $pid");
                        ...
                        if($rw['status']=='completed') {
                            ...
                            echo '<p><span>Project Completed</span></p>';
							...
							echo '<div id="ez" style=""><form method="post" action="dealsview.php?pid='.$pid.'"></br><input type="hidden" name="id" value="'.$rw['id'].'"><input type="text" name="refno" placeholder="Ezcash Refernece No" required></br><button type="submit" name="ez">Ezcash</button></form></div>';
							echo '</br><i>Pay By Bank Deposit - Upload you Deposited Slip file:</i></br>';
							echo '<div id="bd" style=""><form method="post" action="dealsview.php?pid='.$pid.'" enctype="multipart/form-data"></br><input type="hidden" name="id" value="'.$rw['id'].'"><input type="file" id="photo" name="photo" accept="image/*" required></br><button type="submit" name="bd">Bank Deposit</button></form></div>';
							
                        }
                        else {
                    ?>
                <form id="acptForm" action="include/acceptdeals.php" method="post" novalidate>
                    <input type="hidden" name="pid" value="<?=$row['id']?>">
				<div>
                <div>
                ...
                ?>
                        <div class="requests">
                            <h3>
                                <a href="viewfreelancer.php?fid=<?=$r['member_id']?>">#<?=ucfirst($r['member_id'])?></a>
                            </h3>
                            <p><?=substr($r['expertise_description'], 0, 25)?></p>
							<p><?=substr($r['expertise_description'], 25, 25)?></p>
							<p><?=substr($r['expertise_description'], 50, 25)?></p>
							<p><?=substr($r['expertise_description'], 75, 25)?></p>
							<p><b>Bid Amount: </b><?=$r['bid']?> LKR</p>
							<p><b>Estimated Duration: </b><?=$r['duration']?> Days</p>
                            <?php
                            if($row['f_user']==NULL || $row['f_user']==0) {
                            ?>
                            <button name="fid" value="<?=$r['member_id']?>">Accept & Hire</button>
                            <?php
                            }
                            elseif($r['member_id'] == $row['f_user']) {
                            ?>
                            <button disabled name="fid" value="<?=$r['member_id']?>">Accepted</button>
                            <?php
                            }
                            else {
                            ?>
                            <button disabled name="fid" value="<?=$r['member_id']?>"> - </button>
                            <?php
                            }
                            ?>
                        </div>
