...
$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = $uid ORDER BY time DESC");

if(isset($_POST["s_title"])){
	$t=$_POST["s_title"];
	$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = '$uid' AND `title`='$t'");
}

if(isset($_POST["s_type"])){
	$t=$_POST["s_type"];
	$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = '$uid' AND `category`='$t'");
}
...
if(isset($_POST["s_employer"])){
	...
	$emp_id = $employer['id'];
	$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = '$uid' AND `e_user`='$emp_id'");
}

if(isset($_POST["s_place"])){
	$t=$_POST["s_place"];
	$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = '$uid' AND `place`='$t'");
}
...
if(isset($_POST["oldJob"])){
	$result = $mysqli->query("SELECT * FROM job WHERE `f_user` = '$uid'");
}
			...
			<div class="searching">
				<form action="myprojects.php" method="post">
					<div class="s_container">
					  ...
					  <center><button type="submit">Search by Project Title</button></center>
					</div>
				</form>
				...
				<form action="myprojects.php" method="post">
					<div class="s_container">
					  ...
					  <center><button type="submit">Search by Project Type</button></center>
					</div>
				</form>
				<form action="myprojects.php" method="post">
					<div class="s_container">
						<input type="text" name="s_employer">
						<center><button type="submit">Search by Client</button></center>
					</div>
				</form>
				<form action="myprojects.php" method="post">
					<div class="s_container">
				...
				<form action="myprojects.php" method="post">
					<div class="s_container">
					  <center><button type="submit" name="recentJob">See all recent accepted Projects first</button></center>
					</div>
				</form>

				<form action="myprojects.php" method="post">
					<div class="s_container">
					  <center><button type="submit" name="oldJob">See all older accepted Projects first</button></center>
					</div>
				</form>
			</div>
			<div class="joboffers">
				  ...
				  <div>
						  ...
						  if ($result->num_rows > 0) {
								...
								while($row = $result->fetch_assoc()) {
									...
									$ref_job = $row["title"];
									$mydeals = $mysqli->query("SELECT * FROM project WHERE `ref_job` = '$ref_job'");
									if($mydeals->num_rows > 0){
										$numdeals = $mydeals->num_rows;
									}
									else{
										$numdeals = 0;
									}
									...
									echo '
											...
											'.$type.'</br>
											<b>Client:</b> <a href="viewclient.php?cid='.$row['e_user'].'">'.$e_username.'</a></br>
											<b>Project Budget:</b> '.$budget.'</br>
											<b>Project Status:</b> '.$status.'</br>
											<b>Project Location:</b> '.$place.'</br>
											<b>Posted on:</b> '.$timestamp.'</br>
											<b>File:</b> <a href="'.$file.'">'.$source.'</a></br>
											<b>Deals made:</b> '.$numdeals.'</br>
											<b><a href="dealspost.php">Make a Deal</a></b></br>
											</div>
											</div>
											</br>
									';
